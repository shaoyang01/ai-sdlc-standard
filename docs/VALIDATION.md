...
- **Evidence 集合**：docflow 覆盖 `04-实现记录` / `05-代码审核` / `06-知识同步`（review/test 仅 `PASS` 或 `PASS_WITH_RISK`）；`implementation_files` 与 `files` 必须非空、严格升序、无重复且 implementation files 是 files 的子集；所有 Evidence Ref 路径必须出现在 root `files` 中。
