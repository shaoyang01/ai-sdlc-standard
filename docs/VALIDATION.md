...
- 四种 Prompt Mode 名称与 v2 硬限制预算精确（lines / bytes /
  ...
  值被 canonical empty string `""` 结构性替换，投影记账内容零字节，
  绝不对渲染文本 search/replace，projection 从不写入 stdout），完整
  exact payload 始终 inline 于 stdout；
...
- Bounded exact result handoff（PCE_01_BOUNDED_EXACT_RESULT_HANDOFF_G02_
  ...
  payload 始终 inline 于 stdout，仅其值被 canonical empty string
  `""` 结构性替换后参与计量，投影记账内容零字节）；
  ...
  near-boundary 链回归（同一 identity/maximum_bytes/exact payload）
  锁定 PRODUCE compile PASS → ProducedResult.validate PASS → exact
  freeze → CONSUME compile PASS → 解析 stdout 精确保持 identity/
  payload，并机械证明：退役的非空哨兵候选在同一 control envelope 上
  触发 PROMPT_BYTE_LIMIT_EXCEEDED（2071 > 2048）而零内容投影通过
  （2040 ≤ 2048），且 CONSUME projection 的 line/byte/proxy-token
  计数不超过对应 PRODUCE full-output 记账（56≤57 行、2040≤2044
  字节、305≤308 tokens）；invalid UTF-8 payload 的 ProducedResult
  直例回归锁定 FIELD_TYPE_INVALID；
...
- 47 contract fixtures / 69 renderer fixtures / 903 assertions / 47
  registered diagnostics / 10 budget-projection adversarial proofs / 9
  boundary-chain proofs（v2：
  ...
  零内容投影 near-boundary PRODUCE→validate→freeze→CONSUME 链与
  invalid UTF-8 ProducedResult 回归）；
