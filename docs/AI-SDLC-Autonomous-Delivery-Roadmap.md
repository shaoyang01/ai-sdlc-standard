...
- **continuity**：C03-E 当前 Accepted 合同为 [LOOP-CORE-C03-E 规划](LOOP-CORE-C03-E-PLAN.md) v0.4.0、Decision-063 与 Decision-064；A1 已将 Q6 修订为 E0 合同收口包 → 单独授权 E2-P 三 Agent `PROVIDER_REACHABILITY_ONLY` 预检 → E1～E4 runtime 实施包 → E5 验收 → 下一 C05 分段授权。Decision-090（Roadmap v2.5.0）在真实 CLI run8 前新增受控前置节点 `C03-E-PRE-RUN`，冻结顺序：D-088-01 四类项目初始化/迁移重基线与收口 → D-090-01 手动/runtime 共同语义合同 → D-090-02 手动主路径修复并达到 `MANUAL_OPERATIONAL` → 按共同合同恢复/调整 D-087-01..05 → D-090-03 runtime manifest 投影与双路径对齐 → D-090-04 离线 parity 验收 → 真实 CLI run8 → ledger 关闭 + 四仓传播。**2026-09-08 工作归属校准（v2.6.0）**：D-087-01..05 的主干修复归属 **D087**（G4）；D-088-01/G1 初始化器的治理语料收编与缺失生成归属 **D091**；既有 Skill/手动执行面一致性修复归属 **D092**——三者均为上述既定工作包的执行与收口记录，不新增顶层 Requirement 或主线阶段；各包的执行状态、复审结论、授权消费与集成事实以 Decision index、收口治理记录与 Control Plane STATE 为准，本 Roadmap 不维护。剩余衔接保持：D-090-03 → **G5**（差量承接见 [G5 剩余计划](reports/decision-090-g5-remaining-plan.md)）→ D-090-04/**G6**（离线 parity 验收）→ 单独授权真实 CLI run8 → 按实际证据做 C03-E 完成判断 → 单独授权下一条真实需求 C05；真实 run8 不自动等于 C03-E 或 C05 通过。四类项目初始化能力的既定验证要求（Decision-090 冻结矩阵）不降低；多个实际业务仓的正式收编执行与 `pending_confirmation` 语料裁决是 D091 名下的独立 Owner 裁决事项，既不视为自动完成，也不构成 G5 或 run8 的前置条件。Decision-087 的 result model、finding materialization、production entry 三条 seam 保留；PWR 判定继续按 Decision-086 自动放行，不恢复人工风险验收仪式。冻结工作包原文、文件面、测试矩阵、完成门和恢复步骤见 [Decision-090 C03-E-PRE-RUN 执行计划](reports/decision-090-c03e-prerun-governance-plan.md)（FROZEN_PLANNING_BASELINE 原文保留，交付差量由带日期承接说明与 G5 剩余计划表达）。Roadmap 只保存稳定顺序、依赖与工作归属，不保存当前授权或执行状态；每次恢复必须从 Decision index 与 Control Plane STATE 解析当前 live authorization、是否已启动、暂停和下一转换，不能从方案 Accepted 或本段文字推断授权。若代码审核改变已批准行为、架构或验收事实，必须回流 C02；不得把 direct CLI 可达性、人工 Agent 切换、shadow executor 或执行者自述当作自主闭环通过。
...
- **承接归属（2026-09-08 校准）**：D-088-01 → G1 需求重基线与 **D091**（治理语料收编与缺失生成）；D-090-01/02 → Decision-090 治理线；D-087-01..05 → **D087**（G4 主干修复）；D-090-03/04 → G5/G6 剩余包（[G5 剩余计划](reports/decision-090-g5-remaining-plan.md)）。各包执行与收口状态以 Control Plane STATE 及收口记录为准，本节点 completion_contract 不变。
...
| --- | --- | --- | --- |
| 2.6.0 | 2026-09-08 | User-directed governance calibration | C03-E 承接校准：清除 continuity 中作为当前事实出现的过期授权描述（`GW_VERTICAL_REBUILD` 的消费作为带日期事实归收口记录与 STATE，Roadmap 不维护第二份动态状态）；登记工作归属——D087=G4 主干修复、D091=D-088-01/G1 初始化器的治理语料收编与缺失生成、D092=既有 Skill/手动执行面一致性修复，均不新增顶层 Requirement 或主线阶段；区分四类初始化验证要求（不降低）与业务仓正式收编执行/`pending_confirmation` 裁决（独立 Owner 事项，不作为 G5 前置）；固化主线衔接 G5 → G6 → 单独授权真实 run8 → C03-E 完成判断 → 单独授权 C05（run8 不自动等于通过）；新增 [G5 剩余计划](reports/decision-090-g5-remaining-plan.md) 承接 D-090-03 差量。产品目标、Core/Advanced 边界、七节点单轨、solution-gate 双角色隔离与人工 Git 边界不变。 |
