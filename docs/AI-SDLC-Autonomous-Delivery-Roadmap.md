...
- **objective**：使任意已支持 Agent 入口能以同一套产物和 Gate 启动或恢复一个单仓需求，并在 Direct 或 Speckit 路径中完成设计、实现、审核和验收，交付人工 Git 交接包。
- **rationale**：需求和设计细节散落在不同聊天或 Agent 上会导致遗漏；当前首要价值是可靠完成设计—开发—验收循环，而不是无人监督地发布远程 Git 变更。
- **inputs_or_prerequisites**：LOOP Core Contract；目标项目适用的 Standard Package/DocFlow/Speckit 合同；已支持 Agent 的实际适配能力；目标仓库事实。
- **expected_output**：可恢复的 Requirement 运行记录、有效产物链、可追溯的 Agent binding 尝试、Direct/Speckit 实现结果、审核与验收结论，以及 `READY_FOR_MANUAL_GIT_HANDOFF` 或如实的 `blocked`/`failed` 结果。
- **depends_on**：无上游 Roadmap Requirement。
- **scope**：C01～C05 定义的统一入口、产物协调、Direct 路径、Speckit 投影和真实单仓验收。
- **out_of_scope**：自动 commit/push/PR/Ready/merge/发布；为远程 Git 副作用设计的恢复；多仓事务；平台化队列、UI、daemon、registry 或新 Provider 接入。
- **completion_contract**：C01～C05 均按各自完成合同完成；至少一个真实单仓需求在其所选路径中证明完整产物链、有效 Re-Gate、可追溯 binding 与人工 Git 交接；不存在把远程 Git 副作用误写为 Core 成功条件的残留合同。
- **continuity**：C01～C05 的子项共同覆盖 Core；若执行中发现新的 material outcome，必须暂停受影响范围、受控重排本父目标的子项覆盖后再恢复，不能通过会话或 Handoff 隐式扩展。
...
- **definition_status**：`DEFINED`
- **definition_provenance**：`user_accepted`
- **objective**：把需求摘要、技术方案、方案挑战、方案审核、路径决定、实现、代码审核和测试验收的当前有效版本组织为可恢复的协调链。
...
- **scope**：来源冲突/缺失处理；产物版本与引用；挑战/审核；Development Path Decision；最早受影响节点 Re-Gate；有界失败、暂停和恢复。
- **out_of_scope**：以聊天记忆替代产物；将普通执行尝试提升为 Roadmap 阶段；直接修改远程 Git。
...
- **continuity**：业务目标、范围、验收或来源冲突回流需求摘要；架构、接口、数据、异常、兼容性和风险问题回流技术方案；实现错误仅在不改变已批准行为时可停留在实现层；审核/测试揭示方案缺口必须回流方案并重新挑战、审核和复核。

### LOOP-CORE-03 — Direct Single-Repository Delivery and Manual Handoff

- **id**：`LOOP-CORE-03`
- **type**：Sub-requirement
- **parent**：`LOOP-CORE-00`
- **title**：Direct Single-Repository Delivery and Manual Handoff
- **definition_status**：`DEFINED`
- **definition_provenance**：`user_accepted`
- **objective**：当 C02 的有效方案选择 `DIRECT_IMPLEMENTATION` 时，在一个真实目标仓库中完成受约束的实现、文档治理、代码审核和测试/验收，并交付人工 Git 交接包。
- **rationale**：近期需要首先证明完整的单仓产品闭环可用；工作区改动成功不能被误读为远程交付成功。
- **inputs_or_prerequisites**：C02 的审核通过方案、路径决定、有效 finding/Gate；目标仓库事实；适用测试、代码审核和文档治理约束。
- **expected_output**：工作区改动、实现记录、代码审核、测试/验收结论、未执行项、残余风险、恢复说明和 `READY_FOR_MANUAL_GIT_HANDOFF` 或明确失败结果。
- **depends_on**：`LOOP-CORE-02`。
- **scope**：Direct 实现；文档治理；实现错误和方案缺口的 Re-Gate；代码审核、测试/验收与人工 Git 交接清单。
- **out_of_scope**：commit、push、Draft PR、Ready、merge、发布；以未验证测试或历史 CI 替代本次验收。
...
- **continuity**：C03 定义 Core 的共同交付尾部。若代码审核或测试改变已批准行为、架构或验收事实，必须回流 C02；不得把局部修复当作完整闭环通过。

### LOOP-CORE-04 — Speckit Projection and SDD Integration
...
- **inputs_or_prerequisites**：C03；当真实需求选择 Speckit 路径时还需 C04；真实需求来源、目标仓库和适用执行授权。
- **expected_output**：可复核的真实 run evidence、至少一次有效 Re-Gate、binding/输入版本追溯，以及最终人工 Git 交接包或如实的失败/阻塞结论。
- **depends_on**：`LOOP-CORE-03`；`LOOP-CORE-04`（仅当验收需求选择 Speckit 路径）；真实需求证据。
...
| Advanced 11～14 | 旧的复杂需求、多仓、运营编号 | 由 C04、C05、Advanced 01～03 以 Shared §14 合同重新表达；旧编号保留在 Git 历史 |
