...
| E4-T5 | attempt workspace 清理/保留：成功提升、失败隔离、未知副作用保留证据并阻塞 | `core/loop-git-workspace.ts` cleanup options `:88`／result `:93`／manager `:224` | E4-T1、E4-T2（CLEANUP_REQUIRED 驱动） | 规划 §6 E4 | **W6b3：实现完成 `02b642a`（35 断言）→ 独立复审 NOT_CLOSED（1 项阻塞 B1）→ B1 已修复 `05d12d2`（41 断言）→ 聚焦复审待回，CP pass-state 未出**。
