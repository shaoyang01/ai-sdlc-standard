...
| E4-T3 | resume lease 覆盖 recovery→claim→spawn→terminal/promotion 窗口 | `core/loop-resume-lock.ts:130` `withResumeLease` + `runtime.ts:394` 调用面 | E4-T1、E4-T2 | 规划 §6 E4 | **W6b1 PENDING（方案已定，Current User 裁决 2026-08-29）**：把 spawn 与 terminal/promotion 决策窗口显式纳入 lease 覆盖范围；并发 resume 至多一个真实 spawn（规划 S12）；保持 `busy_timeout=0` 与 STORE_BUSY 诚实失败语义，**不得退化为阻塞等待**（持锁时间变长是已知代价） | 并发 claim/spawn 互斥测试（两进程 resume 同一 run 至多一个 spawn）+ 反向探针（摘掉窗口包裹 → 测试转红） |
| E4-T4 | `human_action_required` 机器可读 artifact，reason 仅限 6 个合法码；`SWITCH_AGENT_REQUIRED`/`SHADOW_FALLBACK_REQUIRED` 非法 | 新增 artifact kind `human_action_required`：`core/loop-artifact-store.ts:14` 联合类型 **+ `:34` LOOP_ARTIFACT_KINDS 数组（两处必须同步）**；对接 `core/loop-capability-execution.ts:102` 的 `humanActionRef` 锚点 | E4-T1 | 规划 §6 E4 | **W6b2 PENDING（裁决：新增 kind，不复用既有 kind）**：6 合法码 `MISSING_BUSINESS_FACT`／`SOURCE_CONFLICT`／`RISK_ACCEPTANCE_REQUIRED`／`PERMISSION_REQUIRED`／`EXTERNAL_SIDE_EFFECT_AUTHORIZATION_REQUIRED`／`MANUAL_GIT_HANDOFF_REQUIRED`（规划 :465-469）；`SWITCH_AGENT_REQUIRED`／`SHADOW_FALLBACK_REQUIRED` 由 allowlist 拒绝 | 6 码接受 + 2 非法码拒绝 + 大小写变体拒绝 + `humanActionRef` 与 artifact digest 不匹配拒绝 |
| E4-T5 | attempt workspace 清理/保留：成功提升、失败隔离、未知副作用保留证据并阻塞 | `core/loop-git-workspace.ts` cleanup options `:88`／result `:93`／manager `:224` | E4-T1、E4-T2（CLEANUP_REQUIRED 驱动） | 规划 §6 E4 | **W6b3 PENDING（裁决：本轮就接 wip digest 越界检测，不留到 C-T1）**：三态决策 promote（成功）／isolate（失败，保留证据）／block（未知副作用，保留证据并阻塞，不清理不提升）；越界路径→block 由 wip/status digest 与任务允许路径比对判定（规划 :443-444）；**E5 未授权前不得对真实仓库做 promotion** | 三态各一例 + 越界路径触发 block + 反向探针 |
...
> deterministic）** → **W3=E1-T3/T4（loop-run + production identity/preflight，✅ PASS `598cc72`，S1/S2 已清）** → **W4=E2-T7/D-073（A 链冻结标注 + spawn 引用图 + B-7 零引用锁定，✅ PASS `f10aef1`）** → **W5=E3-T2（九类无效输出不推进 e2e，✅ PASS `eac94c9`，31 断言）** → **W6a=E4-T1+T2（process evidence 固定字段 + recovery 五分类，✅ PASS 独立复审零阻塞，68 断言，CP pass-state PR #22）** → **W6b1=E4-T3（resume lease 覆盖 recovery→claim→spawn→terminal/promotion 窗口）** → **W6b2=E4-T4（human_action_required 六合法码 + 新增 artifact kind）** → **W6b3=E4-T5（attempt workspace 三态 + wip digest 越界检测）** →
