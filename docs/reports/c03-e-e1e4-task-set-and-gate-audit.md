...
> **W1=E2-T8（Q1 绑定，✅ PASS，独立复审 R2 `a698808`）** → W2=E2-T6（gateway 开关，默认【进行中】
...
- **B1（阻断真实激活）**：INITIAL_BINDING_REGISTRY 的 intake 原绑 codex、与 Q1
  目标 kimi 不一致（E2-T8）。**Q1 对齐子项已闭合**——W1（`7f36b8d` + R1 修复 `10b798c`）经独立复审
  **R2 判 PASS（`a698808`）**。仅剩 W2 把关：接线开关必须**默认 deterministic shadow**（真实激活仍未
  授权），W2 落地并确认默认值后 B1 整体解除；production gateway 真实派发前若 registry 未对齐，adapter 仍抛 BINDING_MISMATCH。
