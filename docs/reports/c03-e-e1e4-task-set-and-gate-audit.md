...
| E4-T5 | attempt workspace 清理/保留：成功提升、失败隔离、未知副作用保留证据并阻塞 | `core/loop-git-workspace.ts` cleanup options `:88`／result `:93`／manager `:224` | E4-T1、E4-T2（CLEANUP_REQUIRED 驱动） | 规划 §6 E4 | **W6b3：实现完成 `02b642a`（35 断言）→ 独立复审 NOT_CLOSED（1 项阻塞 B1）→ B1 已修复 `05d12d2`（41 断言）→ **B1 聚焦复审 CLOSED（零阻塞）→ W6b3 ✅ PASS，CP pass-state PR #25 已合并（main `2d2ff53`）。**W6b4（E4-T5 收口）：`1605a84` 把 committed diff 门控到 `allowedPaths !== null`，45 断言 → **聚焦复审 CLOSED（零阻塞）→ ✅ PASS，CP pass-state PR #26 待合并**。建议项 P7（非阻塞，待裁决归属波次）：补 T10c——`allowedPaths: []` + 已提交越界文件 → `CLEANUP_BLOCKED` 且 named diff 恰跑一次**。
