...
| E2-T8 | Q1 binding 对齐：intake/design/task-planning/knowledge-sync→kimi，scan+implementation→codex，verdict+code-review→hermes | binding registry | E2-T6 | Q1 裁决；HANDOFF §3 | IMPLEMENTED `7f36b8d` + R1 修复 `10b798c`（W1：INITIAL 按 Q1 八槽对齐 + 三 Agent 专门 fake runner + Q1 矩阵）；独立复审 R1 提 F1/F2 阻塞、已修复，F3～F5 建议已清（ci-standards 6 步过、tsc 干净、138 文件 0 失败）；**待独立复审 R2 确认 PASS**，R2 PASS 且 W2 开关保持 deterministic 默认后 B1 方解除 | 7×3 矩阵与 Q1 逐格断言 |
...
> W1=E2-T8（Q1 绑定，`7f36b8d` 实现、R1 阻塞修复 `10b798c`、待独立复审 R2 确认）→ W2=E2-T6（gateway 开关，默认
...
- **B1（阻断真实激活）**：INITIAL_BINDING_REGISTRY 的 intake 原绑 codex、与 Q1
  目标 kimi 不一致（E2-T8）。该 Q1 对齐已由 **W1 实现（`7f36b8d`）+ R1 阻塞修复（`10b798c`）**；
  **暂不关闭**——待 W1 独立复审 **R2 确认 PASS**、且 W2 接线开关保持默认 deterministic
  shadow（真实激活仍未授权）后方解除；production gateway 真实派发前若未对齐，adapter 仍抛 BINDING_MISMATCH。
