...
| E2-T6 | production factory 接线 + real-vs-deterministic 选择开关（**默认 deterministic shadow，不得静默切 real**） | 开关工厂 `execution/capability-gateway-source.ts` + `runtime.ts` 装配点（路径 A 的 autonomous-loop/coordinator 按方法二 W4 冻结、不接入） | E2-T4 | 规划 §6 E2；HANDOFF §4.1-4.2；wiring §3 | IMPLEMENTED（W2 `199aeea`，**待独立复审**）：单一工厂 `createCapabilityGateway`，deterministic 逐字复用现状、real 装配 `RealCapabilityGateway`；防静默三条件 fail-closed（显式 source + Q1 注册表 isQ1BindingRegistry + realDeps，缺一抛 `CapabilitySourceError` 不回落 shadow），runtime 封闭 allowlist/枚举、real 与注入 gateway 互斥；默认 deterministic 零行为变化（tsc 干净、ci-standards 6 步、139 文件 0 失败、新测 16 断言）。real 仍休眠：无生产入口传 real（W3 loop-run），D-071 保持 | 选择开关默认值断言；shadow 回归全绿 |
...
> **W1=E2-T8（Q1 绑定，✅ PASS，独立复审 R2 `a698808`）** → W2=E2-T6（gateway 开关，已实现 `199aeea`、待独立复审，默认
...
- **B1（阻断真实激活）**：INITIAL_BINDING_REGISTRY 的 intake 原绑 codex、与 Q1
  目标 kimi 不一致（E2-T8）。两个子项均已实现：**Q1 对齐**——W1（`7f36b8d`+`10b798c`）经独立复审 **R2 判
  PASS（`a698808`）**；**开关默认 deterministic**——W2（`199aeea`）`createCapabilityGateway` 默认
  deterministic、real 需显式 flag + Q1 + realDeps 三条件、缺一 fail-closed 不回落 shadow。**B1 待 W2 独立
  复审 PASS 后整体解除**；真实激活（生产入口传 source=real）仍未授权（W3 loop-run / E5 canary 另批）。
