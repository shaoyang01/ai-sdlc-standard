...
> Version: 1.4.0
> Status: ACCEPTED（2026-09-05 接受并冻结为 G1 验收基线；同日 v1.1.0 勘误：A10/A13/A14 经 Current User 裁决改注为双根拒绝格，验收语义以 §2.2/§2.3 为准，无行为变更；2026-09-07 v1.2.0 按 Decision-091 追加治理语料附录；同日 v1.3.0 追加 R4 修复轮行为增量（语料归档分支与 .sdlc 根路径守卫、转换后摘要来源、归档生命周期统一保护）；同日 v1.4.0 按 D091-R4 复审 B1 修订转换回执与失败所有权语义（附录第 8 条），正文行为不变）
...
- 1.4.0（2026-09-07）：D091-R4 复审 B1 修订——附录第 8 条增补「转换回执前置 + 未决残留 INCOMPLETE」失败所有权语义。无新行为面。
- 1.3.0（2026-09-07）：D091-R3 复审 F1-F3 修复轮增量，追加为「v3.1 附录」第 7-9 条：语料输出路径守卫覆盖 RETIRE 归档落点与 `.sdlc` 根本体、转换后期望摘要改由 transformer 对本次写入字节计算、原件归档生命周期统一保护（复用当场验证/暂存独占创建/清理所有权绑定/目录目标非发布）。原有 C1-C10 与附录 1-6 条行为不变。
...
7. **路径守卫全域（R4-F1）**：语料行（C11/C12）的最终落点——TRANSFORM 目的地与
   RETIRE 归档地址——在计划级与 apply 复验均做 `.sdlc` 根至叶子逐组件 symlink 检查；
   `.sdlc` 根本身为符号链接/非目录时，INIT/AUDIT 跳过全部语料骨架生成（提示 + 正式
   报告记录），adoption/LEGACY 计划级阻塞（`CORPUS_DEST_BLOCKED`）。PRESERVE（已收编
   且已归档一致）维持零写入 no-op 裁断。
8. **转换后摘要来源与失败所有权（R4-F2 / R5-B1）**：语料行的回滚期望摘要是 transformer
   对**本次实际写入字节**在内存中计算并返回的 SHA-256（`total\tsha256` 回执），绝不重新
   读取目的地上可能属于后来者的内容；回执在目的地写入成功后**立即**输出——其后任何步骤
   失败（规则日志追加等），调用方仍凭该回执登记所有权，回滚将本事务转换产物作为自有对象
   撤销，报告保持如实的完整回滚。回执缺失或不可信时（写入可能未发生/部分发生/输出被
   污染），目的地按未决残留处理：不重采样、不无条件删除；可证明原始字节完好则完整回滚，
   否则失败报告如实降级为 INCOMPLETE 并列出来自地。转换完成后目的地被后来者接管的，仍按
   所有权冲突处理——后来者保留、源恢复、冲突入 `rollback.ownership_conflicts`。
9. **归档生命周期统一（R4-F3）**：原件归档复用前当场以备份字节验证（计划的字节一致性
   是计划时快照，过期即冲突并失败回滚，后来者归档永不误绑/误删）；暂存文件以 noclobber
   独占创建（既存普通文件/链接永不截断、永不跟随、永不发布；清理登记仅加入独占创建
   成功的对象）；`mv -n` 将暂存移入晚到目录时按所有权证明撤销嵌套移动；已发布归档的
   失败清理逐项验证 digest 后才删除（`mig_archive_remove_owned`），不匹配者保留并记
   `rollback.ownership_conflicts`。
