...
- **W6b4 = E4-T5 收口：committed diff 门控（实现方在 W6b3 PASS 之后自查发现，非复审方提出）**。`1605a84`，45 断言 → **独立聚焦复审 CLOSED（零阻塞）→ ✅ PASS（2026-08-30）→ CP pass-state PR #26 已合并（main `b9ccd25`）**。复审 prompt：`docs/reports/c03-e-w6b4-focused-review-prompt.md`。
  ...
  - **W6b4 聚焦复审核心证据（CLOSED 零阻塞，2026-08-30 回收）**：① **门控正确性 a)–e) 全部 CLOSED**：`changedPaths` 唯一读取点在 `core/loop-git-workspace.ts:300-301`（被 `allowedPaths !== null` 门控），数据流无漏口；`allowedPaths === null` 调用方门控前后行为**逐字段一致**，唯二可观测差异就是改动本意（少一个子进程、消掉 `_gitR` 的 `GIT_COMMAND_FAILED` 失败面）；`committed=""` 与「跳过」完全等价；`allowedPaths: []` 实测仍跑 diff 并正确 block（**未过度收紧**）；校验（`:459-465`）严格先于门控（`:540`），无短路依赖。② **回归矩阵**：`loop-git-workspace` 110、`loop-w6b3` 45、`loop-codex-implementation-adapter` 354/354、`loop-w6b2` 83 全绿。③ **探针**：P6 去门控 → 恰 T10a（`saw 1`）红、T10b 绿，与实现方口径逐字一致；P8 证明 `--name-only` 区分条件承重；P7 见下方建议项。④ **diff 核验**：`1605a84` 恰两文件（生产 +10/-1 其中 8 行注释；测试 +56），**W1～W6b3 冻结面零改动、real 休眠面零改动**。⑤ **复审方环境全套件 146 文件 / 0 失败 / exit=0 / 320.6s**；两个已知 broker 缺口文件与 adapter flaky 在复审方环境均不复现，维持「环境能力缺口／观察项非归因」口径。
  - **建议项 P7（1 条，合同外、非阻塞，待裁决归属波次）**：现有测试网抓不住「门控误写成 `allowedPaths.length === 0` 也拒」这种**过度收紧**——T10b 用 `["src"]`，而空数组 `[]` 目前靠 adapter 层（`tests/loop-codex-implementation-adapter.test.ts:953`）拒为 `INVALID_INPUT` 兜底，**到不了 cleanup**。建议补一条 **T10c**：`allowedPaths: []` + 已提交越界文件 → `CLEANUP_BLOCKED` 且 named diff 恰跑一次。单处改动即可。
  - **复审方过程发现（实现方已认领，纪律已修正）**：复审开始时**仓库并不干净**——HEAD 停在 `e8e86ce`，且挂着**两处实现方上一轮探针的残留改动**（w6b3 测试文件的 T-probe 块、`loop-git-workspace.ts:540` 的去门控形态）。复审方已 `git checkout --` 还原后再切基线，结论不受影响。→ **纪律修正：反向探针一律在临时 `git worktree` 里跑，禁止在主工作树留痕**；探针结束后必须 `git status --porcelain` 留证。
