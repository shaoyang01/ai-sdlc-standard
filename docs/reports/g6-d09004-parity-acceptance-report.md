...
> 实施分支：`feat/g6-t2-m2-scenarios`（未上主线；M1 已由 PR #195/#196 合入 `feature/loop-runtime-v1` @ `b8923fc`；M2 收口单 PR #197 已开，base `feature/loop-runtime-v1`）。**数据基线约定（R11-H2 确立）**：本报告全部实测数字以「数据基线 commit」为准——即报告实测当时的最后一个代码提交；其后的 docs 同步提交只改文档，不改数字。当前数据基线 = `2ec9ecb`（b8923fc..2ec9ecb 实算 **64 commits**，本行所在 docs 提交为其后的同步提交）。R13-H2 记入：R12-H1 的代码提交曾误冻改写中的行为负向套件（导入已删模块、tsc 退出 2），补修于 `9a3e4a5`——冻结 HEAD 不得携带未提交修复，此事故与修复均在 §6.4 轮次表可查。R14-H1 的代码提交为 `e0a697e`（覆盖审计器重写为执行核验版），R15-H5 的代码提交为 `ce8b61c`（执行事件核验 + 存在性 pin）；R16-H6/H7 的代码提交为 `2ec9ecb`（双路捕获 + 严格事件解析 + 印行/事件逐条对应），其后的报告同步只改文档。
...
- **诚实性仪器**（R1/R2/R10/R11/R12/R13/R14/R15/R16 九轮加固）：runner 输出 ① dim-9 已知原因桶 pin（必须恰等于完成场景数且签名精确，否则硬失败）；② **非 dim-9 分歧计数必须为零**（任何非 dim-9 分歧即回归，独立于桶）；③ 负向矩阵（比较器 38 项 + 行为 **54** 项）钉死 fail-closed 面，且仪器全部分母由**封签账本**（frozen-ledger.json 数据 + ledger-guard 代码封签常量，R12-H1：单改数据 seal mismatch、逐 ID 双向核对、settle 结构化）与**独立覆盖审计器**双层锚定。审计器演进：R13-H1 建（结构布线 + 信任自述覆盖行）→ R14-H1 重写为执行核验（自算封签互核 + 包装探针 + 子进程退出码/行数/场景 ID）→ **R15-H5 重写为执行事件核验**：断言/场景/pin 在求值点发结构化事件（NDJSON，流路径由审计器指定），审计器核验事件本身（分组条数、场景 ID 集合、pin 观测值对账本）+ 印行与事件交叉核对 + guard handle 在 settle 时封闭（settle 后写入抛错）；**R16-H6/H7 补严**：spawnSync 成功/失败双路均捕获 stderr、事件流严格解析（损坏/异物行按行号报出即失败）、印行与事件**逐条对应**（多重集全等而非仅计数；矩阵场景事件携带 id 与 message）。R15 的三种新形态（53 实跑+1 伪造绿行、49 场景+1 伪造 ID 行、settle 后真实红断言）全部被拒；**仪器存在性测试**（R15 建议项落地）保证审计器/账本/守卫/入口被删时全量回归可见失败。
...
| 负向（R1-H1 真实 handoff / R2-H1 ABSENT / R3-H1+R4-H1+H2 WP-1 波 / R1-H2 崩溃篡改 / R8-H4 三态禁写 / R9-H2 回滚后拒判 / R10-H1+R12-H1 账本守卫） | `node --import tsx tests/g6-parity-behavior-negative.test.ts` | **54 passed / 0 failed**（10 组，结构化 settle；R13-H2 后在冻结 HEAD 可运行） |
| 负向覆盖审计（R13-H1 建 / R14-H1 / R15-H5 执行事件核验 / R16-H6·H7 双路+逐条对应） | `node --import tsx tests/g6-negative-coverage-audit.test.ts` | **11 passed / 0 failed**（自算封签互核 2 + 包装合成体探针 5（含 settle 后 handle 封闭）+ 四入口执行事件核验 4：事件分组条数/场景 ID 集合/pin 观测值对账本 + 印行与事件逐条对应 + 严格事件解析 + 成功/失败双路 stderr 捕获） |
...
`npm test`（并行，**173 文件**——含 R13-H1 新增、R14-H1/R15-H5 重写为执行事件核验的覆盖审计器与 R15 建议落地的仪器存在性测试）：**逐文件断言全绿**；文件级失败 = ① `tests/g6-parity-behavior-matrix.test.ts`——按 A′ 设计红的行为矩阵（退出码 1 是 §1 A′ 声明的预期形态，非失控）；② 已知并行抖动 `tests/loop-codex-implementation-adapter.test.ts`（出现时隔离复跑 220 检查 0 失败恢复；本轮未现）。**更正说明（R8 复审方指出）**：日志末尾的 `Results: 1767 passed / 0 failed` 是 `system-capability-review.test.ts` 的**单文件内统计**，并行 runner 不打印跨文件断言总数——历史多处「全量 1767」均为该误述，本报告予以更正，后续以「逐文件全绿 + 文件级失败清单」表述。
...
- **S-CRASH 中断-重入（R1-H2，2026-09-24；R7-H2/R8-H3/R8-H4/R9-H1/R9-H2 五轮加固）**：见 §3.12 family 形状。runner 断言崩溃事实（interruptedAtBoundary / 零重派 / **manifestCaughtUp（cursor=journal head）** / lostWriteOccurred+redriveByteIdentical / doubleResumeNoOp / refusedManifestStable）并**独立计数**（分母取自 S-CRASH 注册表、必须 6/6 且 crashPoint 齐全——R8-H3：`crashRecovery=null` 曾被无条件放行；R9-H1：分母曾用待核验的 crashPoint、六场景 crashPoint 全缺失时退化为 0/0 无专属失败）。诚实化历程（每轮一个 vacuous/退化路径被独立复审闭合）：R7-H2 前 `redriveByteIdentical` 预置 true、只查存在性（重入投影跳过时五标志全 true 假绿）→ R7-H2 改 cursor=head 比对 + lostWriteOccurred 证分支已执行 → R8-H4 前 STOP 后 epilogue 回写被拒清单 → 改为「任何 manifest STOP 即终态：跳过丢失写/双恢复 epilogue + 钉死被拒字节」→ R9-H2 前 STOP 只在回滚前捕获一次（回滚后重入再遭拒仍进双恢复、凭零 dispatch 记 NO_OP）→ 改为每次重入后重判 STOP + 拒判终止 epilogue + 拒判不记 NO_OP。负向（崩溃拒绝类，共 **5 种注入状态 × 8 条断言**，行为负向 54/0 的组成部分）：① 种子篡改（self-digest，未封印）→ MANIFEST_CORRUPT_STOP；② 窗口篡改三态（self-digest / entry digest / 合法封印但 cursor 超前——后者取一次正常跑的末态投影写入窗口）→ MANIFEST_CORRUPT_STOP ×2 + JOURNAL_MANIFEST_MISMATCH_STOP ×1；③ 回滚后、入口重读前篡改 → MANIFEST_CORRUPT_STOP。每态断言按实际来源分别表述：种子态断言**入口拒判与原因**；窗口三态各断言**入口拒判码 + 驱动不改写被拒字节 + 不记 vacuous 事实**；回滚后态另断言**双恢复分支不执行、不记 NO_OP + 被拒字节钉死**。
  **关于「无已知 vacuous 路径」表述的边界（R9-H3 修订、R10-H2 再修订）**：本报告**不作**无 vacuous 路径的全称断言（该形式已被 R8-H3、R9-H1/H2 两度推翻）；仅陈述**已闭合路径清单 + 逐路径证据归属**（每条路径的闭环证据按真实来源归类，不把未被某测试矩阵覆盖的路径记入该矩阵功劳）：
  ...
  | R9-H1 计数退化为 0/0（分母自指） | **runner 的注册表分母 + crashPoint 缺失专属报错** + **frozen-ledger 单元负向**（丢项/重复/规模/精确四态）+ 本轮变异实证（置空 crashPoint → 0/6 + 六条报错 + 退出 1） |
  | R9-H2 回滚后拒判仍记 NO_OP | **行为负向 1 态**（onPostRollback 注入缝）+ 拒判逐次重判的 runner/驱动断言 |
  | R10-H1 完成/超限/崩溃三注册表无规模守卫 | **frozen-ledger 单元负向 5 例** + **两套矩阵 runner、三类注册守卫**（50 完成 / 4 超限 / 6 崩溃，R12-H1 后改为账本双向 ID 核对）+ 变异实证（删完成场景 → 42/50「8 项丢失」退出 1；删超限场景 → 3/4「1 项丢失」退出 1） |
  | R11-H1 负向矩阵覆盖数可自减为绿（删 5 条 → 49/0 退出 0；删 1 条 D-17 → 37/0 退出 0） | **frozen-ledger 冻结覆盖登记**（行为负向 10 组 / 54 条；比较器负向 5 组 / 38 条，settle 结构化不可旁路）+ 删项变异实证（两套各删 → 「executed 49, frozen 54（5 项丢失）」「executed 37, frozen 38（1 项丢失）」均退出 1） |
  | R12-H1 守卫缺独立锚点（冻结值与受检 runner 同仓同改：同步缩减 54→49+删项、缩组表+删组、旁路 settle、场景常量 50→49+删场景，四类均假绿 49/0 退出 0） | **frozen-ledger.json 封签数据 + ledger-guard 代码封签**（账本为 runner 外的数据文件，内容摘要封签于守卫模块常量——单改数据触发 seal mismatch，双改需同时动数据与代码）+ **双向 ID 核对**（场景注册表逐 ID，非计数）+ **结构化 settle**（settle 内置于 runNegativeSuite，无独立可删调用点）。四类假绿推演实证：改账本总量/组表 → seal mismatch 退出 1；删场景 → 点名 missing 退出 1；settle 旁路 → 当时判「结构性不可达」，R13 被证明仍可经 runner 直调/守卫空转假绿，见下一行 |
  | R13-H1 包装层非强制边界（只改负向 runner：直调 body+删断言 → 37/0 退出 0；守卫替换为空转 → 0 断言退出 0） | **本行曾由 R13-H1 审计器「闭环」，但该闭环被 R14 三种单文件变异证伪**（R14-H1 行）——R13-H1 审计器信的是受检代码的**话**（源码串结构检查 + 套件自印覆盖行），R14 变异直调 body 并伪造覆盖行即通过。真实闭环见下一行（R14-H1 的执行核验审计器）。教训：审计器与被审代码同仓时，凡以「被审方自述」为证据的检查（字符串计数、自印摘要行）都不构成边界 |
  | R14-H1 覆盖审计仍会错误通过（R13-H1 审计器被三种单文件变异绕过：a 直调 body+删断言+伪造覆盖行 → 37/0 退出 0、审计 14/0；b 守卫 settle 打桩 → 0 断言退出 0、审计 14/0；c 产物矩阵删 1 场景+注释注册钉 → 49/0 退出 0、审计 14/0） | **执行核验审计器**（tests/g6-negative-coverage-audit.test.ts，@e0a697e 重写）：① **自算封签**——本文件内重算账本内容摘要，与 ledger-guard 模块的 `FROZEN_LEDGER_DIGEST` 互核（不信任 loader，loader 被桩掉也洗不白被改账本）；② **包装探针**——对 committed `runNegativeSuite` 灌合成体：精确体过「settled green」、删一条断言抛「group count drift」、空体抛「EMPTY」、重复注册组抛「twice in a row」（settle 被桩掉则探针即失败）；③ **子进程执行痕迹**——四个入口（两负向 + 两矩阵）全部实跑，核验退出码、**实际执行**的断言行数、**行上场景 ID 集合**对账本（行为负向 54 ✓/0 ✗、比较器负向 38 ✓/0 ✗、产物矩阵退出 0 且 50 个 ✓ ID = 账本完成册、行为矩阵退出 1 且 50 个 ✗ ID = 完成册 + 4 个 ✓ ID = 超限册 + 崩溃 6/6 + 桶 50/50 + 非 dim-9=0 + 超限 4/0）。三种变异实证（与复审方同类手法，在隔离 worktree 对 @e0a697e 施加，全部审计退出 1；其中 a 用较温和形态——删 1 条而非 17 条）：a → 行为负向实跑 53 行 ≠ 冻结 54，审计失败；b → 探针「删断言不抛错」失败（套件亦实跑 0 行）；c → 产物矩阵 ✓ ID 集合缺 1 个 ≠ 账本。边界仍为 tamper-evident（伪造需伪造整个子进程执行痕迹），共谋防线仍为独立复审。**注（R15-H5 后）**：该版的「断言行数/场景 ID」仍取自被审代码的**印行**——输出文本不是执行证据，R15-H5 以执行事件核验取代（见下一行）；R14 的三种变异形式仍被事件版拒绝 |
  | R16-H6 成功路径仍丢 stderr（R15-H5 审计器 spawnSync 前身的成功分支只取 stdout：入口退出 0、stdout 54 绿、stderr 1 红 → 审计 11/0 且误记红行 0） | **spawnSync 双路捕获**（成功/失败均取 status+stdout+stderr）+ 逐条对应核对（红行 1 对事件红 0 → MISMATCH）。R16 探针实证：该形态审计退出 1 |
  | R16-H7 印行/事件仅核计数、解析静默丢弃损坏行（同数不同 ID：改一条产物绿行文本 → 印行 50=事件 50 但 ID 文本不符，审计 11/0；合法 settle 后追加 1 条损坏 NDJSON → 解析器静默丢弃，审计 11/0） | **严格解析**（非 JSON/非事件对象按行号报出，审计失败）+ **逐条对应**（印行断言文本与事件文本按多重集全等核对；矩阵场景事件携带 id 与 message）。R16 两形态均审计退出 1 |
  | R16-H5 基线引用失实（PR 两处与报告一行称 a5e15a9 为当前数据基线——该提交与 ce8b61c 同父、非 HEAD 祖先；「须伪造整个事件流」措辞强于实测） | 基线引用改 ce8b61c/2ec9ecb 链；事件流边界措辞按「补写个别一致记录可通过」收窄（见 R15-H5 行边界）；git diff --check 空行修复 |
  | R15-H4 PR 证据归属失实（PR 正文称五条崩溃退化路径「并被负向钉死」，与报告 §6.2 冲突：R7-H2/R8-H3 两条实际由 runner 断言 + 变异推演闭环，负向矩阵中无其独立用例） | **PR 正文五条路径逐条改按 §6.2 归属标注**（R7-H2 = runner manifestCaughtUp/lostWriteOccurred 断言 + R8 轮变异推演；R8-H3 = runner 注册表计数与 null 专属报错 + R8/R9 轮变异推演；R8-H4 = 行为负向 3 态；R9-H1 = runner 注册表分母 + crashPoint 缺失专属报错 + frozen-ledger 单元负向 5 例；R9-H2 = 行为负向 1 态 onPostRollback） |
  | R15-H5 执行痕迹判据仍错误通过（R14-H1 审计器按 ✓/✗ 文本行计数并提取 ID、成功子进程只留 stdout：a 入口实跑 53 次断言 + 补打一条普通绿行 → 印行 54=冻结，审计 10/0 退出 0；b 删 1 场景 + 补打一条伪造 ID 行 → 印行 50=账本，审计 10/0；c settle 后经捕获 handle 写真实 h.ok(false) → stderr 红行、入口仍退出 0，审计 10/0——「实际执行」结论被推翻） | **执行事件核验**（tests/g6-parity/event-stream.ts + ledger-guard + 两矩阵 + 审计器，R15-H5 修复 @ce8b61c、R16-H6/H7 补修 @2ec9ecb）：① 断言/场景/pin 在**求值点**发结构化事件（NDJSON，流路径由审计器在子进程环境指定，审计器自删自身环境变量防泄漏）；② guard handle **settle 时封闭**——settle 后 ok/group 抛「after settle」（c 形态入口崩溃退出 1）；③ 注册 pin 事件由 assertScenarioLedger **内部**发出（注释掉 pin 调用即无事件）；④ 审计器核验事件本身（负向分组条数恰冻结、场景 ID 集合对账本 50/4/6、pin 观测值 50/50、0、6/6、4/0）+ **印行与事件交叉核对**（无事件 backing 的印行即失败——a/b 形态在此阵亡）+ 成功/失败两路均合并 stderr；⑤ **仪器存在性测试**（tests/g6-parity-instrument-presence.test.ts）——审计器/账本/守卫/四入口被删时全量回归显式失败。三种新形态 + 原三种变异全部隔离 worktree 复刻、审计全部退出 1（见 §6.4 R15 行）。边界为 tamper-evident（R16 修订措辞）：单仓内**补写个别一致的事件记录仍可能通过**（R16 事件伪造推演实测：49 实跑场景补 1 条缺失场景事件 + 1 条注册 pin + 1 条对应绿行即成 50/50）——本层提价的是「令每条印行与事件逐条互洽」的成本形态，不是「必须伪造整条流」；共谋防线仍为独立复审 |
  | R13-H2 冻结提交携带未提交修复（R12-H1 代码提交误冻改写中的行为负向：导入已删模块、tsc 退出 2、套件 MODULE_NOT_FOUND） | **补提交 9a3e4a5 将移植版入冻结 HEAD**（porter 组改接 ledger-guard 逐 ID API：精确/丢项/重复/**额外（unexpected）**各按类抛错 + 密封账本 50/4/6 与 54/38 载荷）；从干净提交复跑 tsc 0、54/0、全量 172 文件唯一红=设计矩阵。纪律教训：冻结 HEAD 不得携带未提交修复——代码提交前必须对**提交物本身**跑 tsc 与受影响套件 |
...
### 6.4 外部独立复审全轮记录（G6T2-R1(M1) + G6-T2-R1..R16(M2)）
...
| G6-T2 R10 | FAIL | R10-H1 完成/超限场景缺独立规模守卫（删 1 个完成场景产物 runner 49/0 退出 0；删超限场景 3/0 无专属错误；桶分母随已运行脚本计数）/ R10-H2 报告 §6.2 覆盖归属夸大（「8 个拒绝态」实为 5 态×8 断言；三条路径误记负向功劳） | H1 registry-guard + 两套矩阵 runner、三类注册守卫（50/4/6）+ 桶分母钉冻结 50 + 单元负向 5 例；H2 §6.2 逐路径证据归属表（R11 判 H1 CLOSED、H2 需修订） |
| G6-T2 R11 | FAIL | R11-H1 负向矩阵覆盖数可自减为绿（删 5 条行为负向 → 49/0 退出 0；删 1 条 D-17 → 37/0 退出 0——R10-H1 未覆盖负向登记）/ R11-H2 报告页首 HEAD/提交数、§1 计数、R7-H2 负向功劳误记、「每态」范围、§6.4 缺 R10、§9 轮次过期 | H1 negative-guard 冻结覆盖登记（6 组/54 与 5 组/38，删项/空集/重复专属硬失败）+ 两种删项变异实证；H2 本报告逐处对账（页首数据基线约定、§1=54、R7-H2 去误记、每态范围收窄、§6.4 补 R10/R11、§9 十一轮） |
| G6-T2 R12 | FAIL | R12-H1 守卫缺独立锚点（冻结值与受检 runner 同仓同改——同步缩减冻结总数+删项、缩组表+删组、旁路 settle、改场景常量+删场景，四类假绿均 49/0 退出 0；R11-H1 的原始删项已闭合但缩减/旁路仍可无声降低覆盖） | frozen-ledger.json 封签数据 + ledger-guard 代码封签常量（单改数据 seal mismatch、双改需动两类文件）+ 场景注册表逐 ID 双向核对 + 负向 settle 结构化（不可旁路）；四类假绿推演全部硬失败已实证——待 R13 复核 |
| G6-T2 R13 | FAIL | R13-H1 包装层非强制边界（只改负向 runner——直调 body+删断言 → 37/0、守卫空转 → 0 断言，均退出 0；R12-H1 的「结构性不可达」被证伪）/ R13-H2 冻结提交携带未提交修复（R12-H1 代码提交误冻改写中的行为负向套件，导入已删模块、tsc 退出 2、MODULE_NOT_FOUND）/ R13-H3 报告与代码基线不同步（页首 ed71783/51 vs 实际、§2 负向 54 不可运行、§6.2 旧守卫功劳归属不实） | H1 负向覆盖审计器（账本独立第三方消费者：封签验证 + 结构布线 + 子进程覆盖行 54/10、38/5 对账）+ 两条绕过推演实证；H2 补提交 9a3e4a5 将移植版入冻结 HEAD 并从干净提交复跑；H3 本报告逐处对账（页首基线 9e9a649/57、§2 实测行、§6.2 归属表 R13 行、§6.4 时序、§9 十三轮 + 审计层）——**R14 复核判 H1 未真正闭环、H3 仍未对齐，见 R14 行** |
| G6-T2 R14 | FAIL | R14-H1 覆盖审计仍会错误通过（R13-H1 审计器的结构串检查 + 自述覆盖行被三种单文件变异绕过：a 直调 body+删断言+伪造覆盖行 → 37/0 退出 0、审计 14/0；b 守卫 settle 打桩 → 0/0 退出 0、审计 14/0；c 产物矩阵删场景+注释注册钉 → 49/0 退出 0、审计 14/0——「单文件修改仍需改审计器才可错误通过」的表述被推翻）/ R14-H3 报告与 PR 证据表述未完全对齐（§6.2 R13-H1 行把被证伪的闭环记为证据、「额外（unexpected）」单元被误述为「顶替」、PR 正文「崩溃事实无 vacuous path」全称断言与报告 §6.2 的限定表述冲突） | H1 审计器重写为执行核验（自算封签互核 + 包装合成体探针 + 四入口子进程执行痕迹 ID 核验，@e0a697e）+ 三种变异在隔离 worktree 复刻、全部审计退出 1；H3 本报告逐处对账（§6.2 R13-H1 行改判 + R14-H1 行、诊断名更正、页首基线 e0a697e/60、§2 审计器 10/0、§6.4 本行、§9 十四轮）+ PR 正文限定化——**R15 复核判 H1 仍有残余（行计数即输出文本）、H4 PR 归属未对齐，见 R15 行** |
| G6-T2 R15 | FAIL | R15-H5 执行痕迹判据仍错误通过（R14-H1 审计器按印行计数+成功只留 stdout：53 实跑+1 伪造绿行、49 场景+1 伪造 ID 行、settle 后真实红断言，三种均审计 10/0 退出 0）/ R15-H4 PR 证据归属失实（五条崩溃路径「并被负向钉死」与 §6.2 逐路径归属冲突） | H5 执行事件核验（求值点结构化事件 + handle settle 封闭 + pin 事件内置 + 印行/事件交叉核对 + 双路 stderr + 存在性测试，@ce8b61c），六种形态 worktree 复刻全部审计退出 1（53+伪造绿行 10/1、settle 后红断言 10/1、直调 10/1、删场景+注销钉 10/1、49+伪造 ID 行 10/1、settle 打桩 5/6），基线 11/0；H4 PR 五条路径逐条标注真实归属——**R16 复核：六指定形态 CLOSED，但另构造两形态（成功路 stderr 红行、同数不同 ID/损坏事件行）错误通过，另判 R16-H6/H7/H5 三项 FAIL，见 R16 行** |
| G6-T2 R16 | FAIL | R16-H6 成功路径仍只取 stdout（退出 0 入口 stderr 红行不可见，审计 11/0 误记红行 0）/ R16-H7 印行与事件仅核计数 + 解析静默丢弃损坏行（同数不同 ID、settle 后追加损坏 NDJSON 均审计 11/0）/ R16-H5 基线引用失实（a5e15a9 非 HEAD 祖先、「须伪造整个事件流」强于实测） | H6 spawnSync 双路捕获 + H7 严格解析与逐条对应（@2ec9ecb），基线 11/0，三新形态（stderr 红行/同数不同 ID/损坏事件行）worktree 复刻全部审计退出 1，旧形态回返仍拒；H5 基线引用与措辞对齐 + 空行修复——待 R17 复核 |
...
1. **全部离线场景通过（归一化后两面等价）**：**BLOCKED（等 R-G6-01 生产修复）**。已达成面：产物层 50 场景九维中 3/4/5 维全等（含 corrupt 单级 fail-closed）；行为层 dims 1/2/6/7/8 全 MATCH、非 dim-9 分歧 0（R6 外部独立复跑核验）；超限单列 4 场景轨迹 + durable 终态全过；十六轮外部复审后仪器面（负向 38+54 项、桶 pin、非 dim-9 计数、崩溃事实独立计数、**场景注册表规模守卫（R10-H1）**、**负向矩阵冻结覆盖登记（R11-H1）**、**冻结账本独立锚点（R12-H1：账本数据封签 + 守卫代码封签 + 逐 ID 双向核对 + 结构化 settle）**、**负向覆盖审计器（R13-H1 建 → R14-H1 执行核验 → R15-H5 执行事件核验 → R16-H6/H7 双路捕获 + 严格解析 + 印行/事件逐条对应——R14/R15/R16 累计九种单文件变异形式全部拒绝）**、**仪器存在性 pin（R15：审计器/账本/守卫/入口被删即全量回归失败）**、**冻结提交完整性（R13-H2：代码提交前对提交物本身跑 tsc 与受影响套件）**）可信。未达成面：**dim-9 final-handoff 在全部 50 个完成场景上 surfaced R-G6-01 分歧**——该判据的完全满足以生产修复为前提（decision record + 生产代码，单独轨道）。故完成门第 1 项状态 = 仪器与产物层全绿、行为层等价性被已路由生产缺陷阻塞，**不得判 PASS，亦不得掩盖**。
