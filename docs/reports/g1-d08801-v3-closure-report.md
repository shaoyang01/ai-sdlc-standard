...
1. **A10/A13/A14 冻结矛盾——已裁决（2026-09-05）**：Current User 维持 D1/D2 双根阻断语义；规格 v1.1.0 已将三格改注为双根拒绝格（验收=阻断零写入，场景 39 承载），人工处置路径为官方出口（清退一侧后重跑；知识迁位走 git mv + create-if-missing 保护）。"向已治理表面迁移"的工具化支持推迟至真实需求出现时做针对性修订。
