...
''');ruby.chmod(0o755)
  ...
  assert row['self_written_dst_remaining']=='Use .sdlc/memory/x\n' and row['status']=='FAILED_ROLLED_BACK'
  rows.append(row)
(ROOT/'results.json').write_text(json.dumps(rows,ensure_ascii=False,indent=2));print(json.dumps({'evidence_root':str(ROOT),'reproductions':rows},ensure_ascii=False,indent=2))
