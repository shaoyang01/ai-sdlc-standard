...
2. **R13-H2 冻结提交携带未提交修复（整改方事故）**：R12-H1 的代码提交（4c725f2）误冻**改写中的**行为负向套件——第 22 行仍 import 已删除的 registry-guard、722-757 行仍调旧 API；tsc 退出 2、套件 MODULE_NOT_FOUND、npm test 列为失败文件；修复版留在工作区未提交。补提交 **9a3e4a5**（registryGuardPin 组改接 ledger-guard 逐 ID API：精确/丢项/重复/顶替各按类抛错 + 密封账本 50/4/6 与 54/38 载荷验证）。**纪律教训入交接**：冻结 HEAD 不得携带未提交修复——代码提交前必须对**提交物本身**跑 tsc 与受影响套件。
