...
1. **R15-H5 执行痕迹判据仍错误通过**：R14-H1 审计器按 **✓/✗ 文本行**计数并提取 ID，且成功子进程只保留 stdout——印行仍是**被审代码的输出文本**，不是执行证据。三种新形态全部错误通过（审计 10/0 退出 0）：a 入口实跑 **53 次断言 + 补打一条普通绿行**（印行 54=冻结，退出 0）；b 删 1 场景 + 补打一条伪造 ID 行（印行 50=账本）；c **settle 后经捕获 handle 写真实 `h.ok(false)`**（红行落 stderr、入口仍退出 0，成功路径不捕获 stderr 故不可见）。修复（@ce8b61c，代码）：
   - **event-stream.ts**（新）：求值点结构化事件（NDJSON）——assert / group / settle（guard handle）、scenario（两矩阵判点）、pin（注册 pin + A′ 四项 pin，带观测值）；流路径由审计器经子进程环境变量 `G6_EVENT_STREAM` 指定（临时目录、用后即删；审计器自身进程先删该变量防探针泄漏）；
   - **ledger-guard**：handle 每个求点发事件；**settle 时封闭 handle**——settle 后 ok/group 抛「after settle」（c 形态入口直接崩溃退出 1）；`assertScenarioLedger` **内部**发注册 pin 事件（注释掉 pin 调用即无事件，防止「调用被中立但事件还在」）；
   - **两矩阵**：场景判点发 scenario 事件（含超限循环的直印路径）+ A′ pin 事件（dim-9 桶 50/50、非 dim-9、崩溃 6/6、超限 4/0，观测值随事件发出）；
   - **审计器**：四入口改为**事件核验**——事件分组条数恰冻结、场景 ID 集合对账本 50/4/6、pin 观测值对账本、**印行与事件交叉核对**（无事件 backing 的印行即失败）、成功/失败两路均合并 stderr；探针增加「settle 后 handle 封闭」一项；
   - **g6-parity-instrument-presence.test.ts**（新，R15 非阻塞建议落地）：审计器/账本/守卫/event-stream/四入口被删 → 全量回归显式失败（修复「删审计器→171 文件、失败清单不变、证据静默消失」）。
   验证：tsc 0；基线审计 **11/0** 退出 0（封签 2 + 探针 5 + 四入口事件核验 4）；存在性 **8/0**。三种新形态 + R14 原三种变异在隔离 worktree（@ce8b61c 单文件修改；其中「53+伪造绿行」「settle 后红断言」两探针在前一版本 a5e15a9 上运行——两版仅差审计器失败消息措辞、判定谓词完全一致）复刻，**全部审计拒绝并退出 1**：53+伪造绿行 → 0 事件/0 settle（10/1，入口退出 0 也拒）；settle 后红断言 → 入口崩溃退出 1（10/1）；49+伪造 ID 行 → 49 事件≠50 且印行 50≠49（10/1）；直调 body → 0 事件（10/1）；删场景+注销钉 → 49 事件≠50（10/1）；settle 打桩 → 探针四项失败 + 两负向 0 行（5/6）。
2. **R15-H4 PR 证据归属失实**：PR 正文称五条崩溃退化路径「并被负向钉死」，与报告 §6.2 冲突（R7-H2/R8-H3 两条实际由 runner 断言 + 变异推演闭环，负向矩阵无其独立用例）。修复：PR 五条路径逐条改按 §6.2 归属标注（runner 守卫 / 负向用例 / 变异推演三类真实来源分列）。

**验证**（数据基线 ce8b61c）：tsc 0；审计器 **11/0**；存在性 **8/0**；六种探演全部拒绝（退出 1）；行为负向 **54/0**；比较器负向 **38/0**；产物 **50/0**；行为矩阵 A′ 四要素 + 崩溃 **6/6**；全量 **173 文件**逐文件断言全绿，唯一文件级失败 = 按设计红矩阵。改动面 tests/ + docs，生产代码零 diff；b8923fc..ce8b61c 实算 **62 commits**。

**元教训递进（入交接）**：R13-H1——自述（源码串、自印覆盖行）不是证据；R14-H1——自述不可信后改核执行痕迹（退出码/行数/ID）；**R15-H5——印行仍是输出文本，只有求值点发出的结构化事件 + 印行/事件互洽 + 双路流合并才是执行证据**。每轮复审都在提高「伪造通过」的成本形态，单仓 tamper-evident 边界不变，最终防线仍是独立复审每轮重放。

**剩余（每步先请 Current User 确认）**：PR #197 正文同步（R15 轮记录 + 事件核验改动面 + CI 现状）→ 独立复审（R16，范围 = R15-H5 六形态拒绝复算 + R15-H4 归属对账 + 全 CLOSED 面不回归 + 事件流元层次）→ PASS 后请 Current User 授权合并。

## 2026-09-25 家用机会话（续）：R14 复审 FAIL → R14-H1 修复 + R14-H3 表述对账（@e0a697e 已推）

**R14 判定 FAIL**（外部只读副本 @7c462f6 = R13 三修复后的冻结基线；R13-H2 闭环判符合；三项阻塞）：

1. **R14-H1 覆盖审计仍会错误通过**：R13-H1 审计器信的是被审代码的**话**——源码串结构检查 + 套件**自印**覆盖行。三种单文件变异全部错误通过（审计 14/0 退出 0）：**a** 直调 body + 删一条断言 + 伪造覆盖行 → 负向 37/0 退出 0；**b** 守卫 settle 打桩 → 0 断言退出 0；**c** 产物矩阵删一个完成场景 + 注释注册钉 → 49/0 退出 0。修复：**审计器重写为执行核验**（tests/g6-negative-coverage-audit.test.ts，@e0a697e）——① **自算封签**：本文件内重算账本内容摘要，与 ledger-guard 的 `FROZEN_LEDGER_DIGEST` 互核（不信任 loader，loader 被桩洗不白被改账本）；② **包装探针**：对 committed `runNegativeSuite` 灌合成体——精确体 settle 绿、删一条断言抛「group count drift」、空体抛「EMPTY」、重复注册组抛「twice in a row」（settle 被桩即探针失败）；③ **子进程执行痕迹**：四个入口全部实跑，核验退出码、**实际执行**的断言行数、**行上场景 ID 集合**对账本（自述行不算）。三种变异在隔离 worktree（@e0a697e + 单文件修改）复刻，**全部审计退出 1**：a → 实跑 53 行 ≠ 冻结 54（9/1 失败）；b → 探针三态全不抛 + 两套负向实跑 0 行（5/5 失败）；c → 产物矩阵 ✓ ID 集合 49 ≠ 账本 50。基线实测（提交前对提交物内容）：审计 **10/0** 退出 0。
2. **R14-H3 报告与 PR 表述未对齐**：§6.2 的 R13-H1 行把被 R14 证伪的闭环记为证据；「额外（unexpected）」单元被误述为「顶替」；PR 正文「崩溃事实无 vacuous path」全称断言与报告 §6.2 的限定表述（已闭合 5 条、不作全称断言）冲突。修复：§6.2 R13-H1 行改判（闭环作废、真实闭环指向 R14-H1 行）+ 补 R14-H1 行；诊断名更正（报告 1 处 + 交接 2 处）；PR 正文改限定表述；页首基线 e0a697e/60；§2 审计器 10/0；§6.4 补 R14 行；§9 十四轮。

**验证**（数据基线 e0a697e）：tsc 0；审计器 **10/0** 退出 0；三种变异审计全部退出 1（隔离 worktree）；行为负向 **54/0**（10 组）；比较器负向 **38/0**（5 组）；产物 **50/0**；行为矩阵 A′ 四要素完整 + 崩溃 **6/6** + 三注册账本钉；全量 **172 文件**逐文件断言全绿，唯一文件级失败 = 按设计红矩阵。CI 现状（R14 复审方指出 @7c462f6）：ci-tests fail = ① 按设计红矩阵（R-G6-01 修复前必然）+ ② hermes-cli-executor-contract CI 并行满载失败（本地隔离 21/0 恢复，资源竞争类抖动，不 import G6 harness）。

**元教训（R14-H1，入交接）**：审计器与被审代码同仓时，凡以**被审方自述**为证据的检查——源码串计数、自印摘要/覆盖行、自报覆盖率——都不构成边界；必须核验**执行痕迹**（退出码、实际产生的输出行、行上 ID）。封签账本/结构化 settle 防的是「改数」，本层防的是「改话」。三种变异复刻手法已成惯例：每轮复审的假绿推演，修复后在隔离 worktree 复刻并全部要求硬失败。

**剩余（每步先请 Current User 确认）**：PR #197 正文/描述同步（R14 轮记录 + 执行核验改动面 + CI 现状如实说明）→ 独立复审（R15，范围 = R14-H1 闭环复算 + R14-H3 表述对齐 + 全 CLOSED 面不回归 + 审计器元层次）→ PASS 后请 Current User 授权合并。

## 2026-09-25 家用机会话：R13 复审 FAIL → R13-H1/H2/H3 修复（@9a3e4a5 + @9e9a649 + @29d053a 已推）

**R13 判定 FAIL**（外部只读副本 @38a9394，三项阻塞；R12 的四类假绿推演本身判符合，但边界证明不足）：

1. **R13-H1 包装层非强制边界**：只改负向 runner——将 `runNegativeSuite` 调用改为直调 body+删一条断言 → **37/0 退出 0**；把守卫函数替换为空转 → **0 断言退出 0**。R12-H1 声称的「settle 结构性不可达」被证伪（wrapper 本身不是 enforced boundary）。修复：新增 **`tests/g6-negative-coverage-audit.test.ts`（负向覆盖审计器）**——冻结账本的**独立第三方消费者**：① 经守卫验证账本封签；② 结构检查（每套负向恰一次 runNegativeSuite 布线、恰冻结组数、两矩阵 runner 的注册钉在位、无已删模块 import）；③ **子进程实跑两套负向**，要求其**由包装生成的覆盖行**（frozen coverage: N assertions across M groups）对账本 54/10、38/5——直调或空转下覆盖行缺失即审计失败。两条绕过推演实证：直调（含伪造摘要行）→ 结构检查 + 覆盖行缺失双失败；空转（38/0 退出 0）→「coverage line MISSING (the wrapper did not run: bypassed or stubbed)」审计退出 1。诚实边界：tamper-evident 非 tamper-proof（伪造需同时改审计器=第三文件），共谋防线仍为独立复审。
2. **R13-H2 冻结提交携带未提交修复（整改方事故）**：R12-H1 的代码提交（4c725f2）误冻**改写中的**行为负向套件——第 22 行仍 import 已删除的 registry-guard、722-757 行仍调旧 API；tsc 退出 2、套件 MODULE_NOT_FOUND、npm test 列为失败文件；修复版留在工作区未提交。补提交 **9a3e4a5**（registryGuardPin 组改接 ledger-guard 逐 ID API：精确/丢项/重复/额外（unexpected）各按类抛错 + 密封账本 50/4/6 与 54/38 载荷验证）。**纪律教训入交接**：冻结 HEAD 不得携带未提交修复——代码提交前必须对**提交物本身**跑 tsc 与受影响套件。
3. **R13-H3 报告与代码基线不同步**：页首 ed71783/51 vs 实际；§2 写 tsc 0、负向 54/0 与冻结 HEAD 实测冲突（套件不可运行）；§6.2 把无法启动的旧守卫单元检查记作闭环证据。修复：页首基线改 **9e9a649/57**（保留数据基线约定 + R13-H2 事故记录）；§2 补审计器行（14/0）、负向行标注「R13-H2 后冻结 HEAD 可运行」、文件数 171→**172**；§6.2 归属表补 R13-H1/R13-H2 两行、旧模块名更正为 ledger-guard；§6.4 轮次表**按时间排序**（R11→R12→R13）+ 补 R13 行；§9 改十三轮 + 审计层 + 冻结提交完整性项；§1 诚实性仪器改六轮加固表述。

**验证**：tsc 0（冻结 HEAD）；审计器 **14/0**；行为负向 **54/0**（10 组）；比较器负向 **38/0**（5 组）；产物 **50/0**；行为矩阵 A′ 四要素完整 + 崩溃 **6/6** + 三注册账本钉；全量 **172 文件**逐文件断言全绿，唯一文件级失败 = 按设计红矩阵。改动面 tests/ + docs，生产代码零 diff；b8923fc..9e9a649 实算 57 commits。
...
- **ledger-guard.ts**（新，代码）：封签是**代码**——`FROZEN_LEDGER_DIGEST` 常量。只改数据 → 内容摘要不符硬失败；改数据+重封 → 模块封签不符硬失败（伪造需同时故意动两类文件）；`assertScenarioLedger` **逐 ID 双向核对**（丢失/重复/额外逐名叫名，非计数）；`runNegativeSuite` **结构化包装**——settle 内置于包装内（无独立可删调用点），每组须跑恰冻结条数，摘要行由实际执行生成（不再写死）；
