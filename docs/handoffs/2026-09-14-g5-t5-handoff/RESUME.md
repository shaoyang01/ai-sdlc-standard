...
| RC4-1 | 头注释声称 "three scenarios" 而 `main()` 执行四个（**刚修完同类问题即复发**） | 头注释改为与 `main()` 严格对应的编号列表（现五场景） |
...
| RC6-1 | 声明与执行不符 | 删死代码 `scenarioAcceptedMixedV9`、头注释与执行对齐 | NOT_CLOSED | 头注释 "three scenarios" 同类复发 → 编号列表（现五场景） |
