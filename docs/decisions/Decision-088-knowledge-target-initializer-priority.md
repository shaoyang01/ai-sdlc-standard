# Decision-088：知识目标初始化器立项与优先级裁决（W-KT-INIT）
...
1. **立项 W-KT-INIT（知识目标初始化器波）**，方向（细节以分析报告为准）：
...
2. **优先级裁决**：W-KT-INIT 优先于 LOOP runtime 冒烟；该缺口关闭前不推进 D-087 顺序中的真实 CLI 冒烟（run8）。D-087 纵向重建波暂停（`GW_VERTICAL_REBUILD` live authorization 保留、不消费，W-KT-INIT 收口后恢复）；E5-L3 冻结、D2 挂账、ladder ② ③ PENDING 边界全部不变。
...
4. **授权边界**：本 Decision 不构成实施授权。W-KT-INIT 实施需 Current User 对实施范围的显式授权；三个业务仓在本波内只做 dry-run 只读验证，正式初始化逐仓单独授权。
...
- roadmap v2.4.0 补记 W-KT-INIT 优先级另行授权（v2.4.0 route amendment 流程先例）；
