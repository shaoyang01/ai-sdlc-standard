# sdlc-requirement-normalizer Skill Contract
...
`sdlc-requirement-normalizer` 是需求入口整理器。
...
- Activity Log
- Missing Artifacts
- Next Step: 执行 `sdlc-specification-writer`
...
- 输出可被 `sdlc-specification-writer` 消费。
- 如果存在待确认事项，必须显式记录。
- 如果待确认事项影响核心 Scope，不得进入 `sdlc-specification-writer` 生成确定方案。
