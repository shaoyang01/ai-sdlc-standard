# specification-writer Skill Contract
...
`specification-writer` 是 Speckit 之外的通用规格生成入口。
...
- 明确哪些内容是事实、哪些是待确认事项。
- 为 `solution-reviewer` 提供可审阅输入。
- 在轻量需求中，让 `01-技术方案` 直接作为规格事实。
- 在进入 Speckit 时，让 `01-技术方案` 成为 `speckit-specify` 派生或同步 `specs/spec.md` 的输入。
...
- 执行业务代码修改。
- 渲染 HTML 样式或发布飞书文档。
- 替代 `speckit-pipeline-confirmed-single` 执行完整 SDD。
- 将不确定业务规则写成确定事实。
...
- `requirement-normalizer` 输出的需求摘要
- 相关业务知识库或代码事实
...
- `specification-writer` 只负责规格内容。
- `docflow-writer` 负责落盘、渲染或发布。
...
- Activity Log
- Missing Artifacts
- Next Step: 执行 `solution-reviewer`
...
- 写入 `01-技术方案` 产物。
- 输出 `manifest.md` 更新建议。
- 在用户明确要求时调用 `docflow-writer` 做输出路由。
...
- 生成 `01-技术方案` 后，必须进入 `solution-reviewer`。
- 未经 `solution-reviewer` 审阅，不得直接进入实现。
- 如果方案内仍存在待确认事项，`solution-reviewer` 应判断是否 `BLOCKED_NEEDS_REVISION`。

## Relationship With Other Skills

### docflow-writer

`docflow-writer` 负责文档路由、格式输出和发布。

`specification-writer` 负责规格语义。

二者不能互相替代。

### solution-reviewer

`solution-reviewer` 消费 `specification-writer` 的产物，输出方案审核和开发路径建议。

### speckit-specify

当需求进入 `speckit-pipeline-confirmed-single` 时，`speckit-specify` 应复用 `specification-writer` 的已审阅产物，派生或同步 `specs/spec.md`。

`speckit-specify` 不应重新解释已经审阅通过的需求。

### speckit-clarify

`speckit-clarify` 只校验残余未决问题。
