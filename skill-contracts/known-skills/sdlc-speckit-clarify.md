# sdlc-speckit-clarify Skill Contract
...
`sdlc-speckit-clarify` 在新流程中只做残余未决问题校验。
...
- 改写方案审核结论。
- 把核心业务问题包装成实现细节。
- 替代 `sdlc-solution-reviewer` 决定开发路径。
...
- `library/{requirement_id}/manifest.md`
- `sdlc-solution-reviewer` 的 Development Path Recommendation
...
- 进入本 Skill 前，核心需求问题已经由 `sdlc-specification-writer` 和 `sdlc-solution-reviewer` 处理。
...
- 无核心未决问题时，可进入 `sdlc-speckit-plan`。
