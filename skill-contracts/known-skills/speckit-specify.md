# speckit-specify Skill Contract
...
`speckit-specify` 在新流程中不再从零理解需求。

它负责：

- 读取已审阅的 `01-技术方案` 和 `02-方案审核`。
- 将 `specification-writer` 的规格事实同步或派生为 `specs/{feature}/spec.md`。
...
- 替代 `specification-writer` 编写技术方案。
- 替代 `solution-reviewer` 审阅方案。
- 重新解释已审阅通过的需求。
- 绕过方案审核直接创建实现计划。
...
- 如果 `01-技术方案` 与 `02-方案审核` 冲突，应停止并回到 `solution-reviewer` 或方案修订。
...
- 创建或更新 `specs/**`。
- 执行 Speckit 创建 feature/spec 的脚本。
...
- 完成后建议进入 `speckit-clarify` 做残余未决问题校验。
