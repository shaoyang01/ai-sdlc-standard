# test-feedback-classifier Skill Contract
...
`test-feedback-classifier` 是测试反馈分类器。
...
| Review Missing | 记录 Review 缺口，建议后续由 `test-feedback-sync` 沉淀到 Review Checklist。 |
...
- 可复用的 Checklist / Schema 缺口交给 `test-feedback-sync`。
