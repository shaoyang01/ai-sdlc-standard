# sdlc-test-feedback-sync Skill Contract
...
`sdlc-test-feedback-sync` 消费已结构化的测试验收产物，判断是否需要回写 Checklist、Schema、manifest 或后续知识同步。

它负责：

- 读取 `sdlc-test-feedback-classifier` 生成的 `05-测试验收` 产物。
...
- 反馈分类未决时，返回 `sdlc-test-feedback-classifier` 补充分类。
...
- 已有可验证实现或测试反馈。
- `sdlc-test-feedback-classifier` 已产出 `05-测试验收`，或用户提供等价结构化分类。
- 方案和方案审核应存在。
