...
input_artifacts:
  ...
  - optional templates/business-domain-l4/{profile}.md only when standard template fallback is explicitly active
output_artifacts:
  - sync report
  - knowledge updates or proposed updates
  - authorized create-if-missing L4 document using project naming and project shape
  - or sync proposal when naming/shape/fallback conditions are not satisfied
...
side_effects:
  ...
  - create authorized missing L4 business-domain documents using project canonical naming and project shape
  - use project-type standard templates only as explicit fallback when allowed
...
blocking_conditions:
  ...
  - create-if-missing cannot resolve project canonical naming or project shape
  - standard template fallback attempted without explicit fallback conditions
...
- 缺失 L4 时的 naming_pattern_source、shape_profile_source、shape_confidence 与 standard template fallback 条件。
...
- 缺失 L4 时必须能解析 naming_pattern_source、shape_profile_source、shape_confidence；standard template fallback 仅在 fallback 条件满足时可用。
...
- 缺少 naming_pattern_source、shape_profile_source 或 shape_confidence 时停止或输出 proposal。
- 只有 fallback 已被明确激活时，缺少 Project Type Profiles 或 selected fallback template 才阻断 fallback。
...
- 修改已授权知识目标。
- 在 create-if-missing 已授权且 L1/L2 已确认时，基于 project canonical naming + project shape 创建 L4 文档；Selected L4 Template 仅在 standard template fallback 激活时用于 fallback template 选择。
...
- 缺失 L4 无 naming_pattern_source、shape_profile_source 或 shape_confidence。
...
- business_domain Sync 已确认目标 L1/L2/L4、owner、entry coverage 状态；缺失 L4 时已记录 create-if-missing 决策、naming_pattern_source、shape_profile_source、shape_confidence 和 fallback 条件。
