# speckit-pipeline-confirmed-single Skill Contract
...
`speckit-pipeline-confirmed-single` 是方案审阅后的可选完整 SDD 路径。
...
- 将 `specification-writer` 的产物同步或派生为 `specs/spec.md`。
- 在实现完成后将稳定业务事实回写到 `.specify/business_domain/**`。
- 在 DocFlow 中形成实现记录和 Sync 状态。

它不负责：

- 替代 `solution-reviewer` 做方案审阅。
- 在方案审核失败时继续推进。
- 从零重新澄清已经审阅过的需求。
- 在 `speckit-clarify` 中扩大需求范围。
- 自动绕过用户确认进入实现。
...
- `solution-reviewer` 输出 `SPECKIT_PIPELINE_REQUIRED`。
- 用户明确要求完整 SDD 流程。
...
当 `solution-reviewer` 输出 `DIRECT_IMPLEMENTATION` 时，默认不启动本 Skill；除非用户明确要求完整 SDD。
...
- `Analyze`：审计 plan/tasks/specs 一致性，不替代 `solution-reviewer`。
- `Implement`：不得实现方案外行为。
...
- 开始修改代码。
- 执行 Sync 回写。
- 对 `speckit-code-doc-reconcile` 使用 `--apply`。
