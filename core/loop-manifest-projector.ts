...
export type ManifestMapFindingRow = Readonly<{
  ...
  first_seen: Readonly<{
    ...
    runtime_locator: {
      ...
      /**
       * O-3 (T2 closure residual, R8-S1): despite the name this carries the
       * producing REVISION id, not an execution id. It is a forensic
       * annotation only — the pairing/lookup predicate never reads it
       * (R8-S1 ruling), so the naming mismatch is documented rather than
       * fixed to keep the frozen provenance schema stable.
       */
...
/**
 * G5-T5 (completion gate #1): the cross-face file format is the manual
 * publisher's markdown template with the canonical YAML in a ```yaml fence.
 * Bare YAML (this emitter's own historical form) stays accepted so documents
 * published before this change remain readable.
 */
export function extractManifestYaml(rawText: string): string {
  if (rawText.startsWith("---\n") || rawText === "---") return rawText;
  const match = /```yaml\n([\s\S]*?)```/.exec(rawText);
  if (match === null) {
    stop("MANIFEST_CORRUPT_STOP", "no embedded yaml block");
  }
  return match[1];
}

/** Wraps canonical YAML in the publisher's exact markdown template so both
 * faces maintain the same file and either face's tooling can read it. */
export function wrapManifestYaml(yamlText: string, requirementId: string): string {
  let md = `# Requirement Manifest: ${requirementId}\n\n`;
  md += "<!-- manual-runtime-semantic-contract v1.0.0 \u00a76.2 self-attesting projection; -->\n";
...
function loadManifestFile(manifestPath: string): ManifestState | undefined {
  ...
  try {
    parsed = parseRubyYaml(extractManifestYaml(rawText));
  } catch (error) {
...
function catchUp(
  ...
  publishAtomically(manifestPath, wrapManifestYaml(dumpRubyYaml(stateToYamlMap(sealed)), sealed.requirement_id));
