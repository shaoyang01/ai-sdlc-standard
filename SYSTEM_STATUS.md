...
170. **Hermes Gateway Real Dispatch Phase-2 Shadow Enablement Guardrail Contract** — ✅ Done (status: guardrail_contract_only; no Phase-2 implementation now; no real guardrail execution now; no guardrail log persistence now; no request type expansion now; no feature flag enablement; no enablement scripts; no CI behavior change; no Runtime behavior change; no Gateway behavior change; no Hermes dispatch eligibility change; current validated request type remains review only; phase-2 shadow targets are code_review and validation; defines guardrail decisions, refusal conditions, fallback behavior, and prohibited guardrail data; operator approval required; automatic enablement blocked; no Hermes default routing; no final review/code_review/validation ownership; no persistence; no raw prompt/artifacts/secrets/stdout/stderr/full CLI output; next PR: Hermes Gateway Real Dispatch Phase-2 Shadow Enablement Rollback Contract)
171. **Controlled Skill Flow Routing / Governance** — Memory-aware routing with audit trail
172. **Graph Kernel Alignment** — code-review / bugfix as lifecycle nodes
