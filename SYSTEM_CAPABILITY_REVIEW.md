...
| Phase-2 Shadow Enablement Contract | contract_only | `execution/hermes-gateway-real-dispatch-phase-2-shadow-enablement-contract.ts` |
...
- Next PR: Hermes Gateway Real Dispatch Phase-2 Shadow Enablement Test Plan
...
| Real adapter rollout remains constrained | Medium | Hermes Gateway real dispatch now has a contract-only phase-2 enablement guard contract, a plan-only phase-2 shadow enablement implementation plan, and a contract-only phase-2 shadow enablement contract defining how a future PR may safely implement shadow-only Phase-2 enablement. Next step is a phase-2 shadow enablement test plan. | Hermes Gateway Real Dispatch Phase-2 Shadow Enablement Test Plan |
| Codex adapter supports only code_generation | Medium | Extend to review/bugfix types | Codex Adapter extension |
| No primary Hermes Gateway ownership | Medium | Phase-2 enablement guard contract, shadow enablement implementation plan, and shadow enablement contract reinforce code_review/validation expansion targets as sidecar-only and forbid final ownership without a separate shadow enablement implementation PR. Next step is phase-2 shadow enablement test plan. | Hermes Gateway Real Dispatch Phase-2 Shadow Enablement Test Plan |
...
**Recommended: Hermes Gateway Real Dispatch Phase-2 Shadow Enablement Test Plan**

Hermes Gateway real dispatch now has a contract-only phase-2 shadow enablement contract defining the exact behavior contract a future shadow-only implementation must satisfy for code_review and validation. The next step is a test-plan-only phase-2 shadow enablement test plan that remains non-executing and non-enabling.
