...
The next intended PR is Hermes Gateway Real Dispatch Phase-2 Shadow Enablement Controlled Rollout Gate. Hermes Gateway real dispatch now has an operator-accepted phase-2 shadow sidecar attachment for code_review and validation that remains feature-flagged, operator-approved, sidecar-only, default-off, and unable to change Gateway primary result or Runtime final_status/routing.
...
**Recommended: Hermes Gateway Real Dispatch Phase-2 Shadow Enablement Controlled Rollout Gate**

Hermes Gateway real dispatch now has an operator-accepted phase-2 shadow sidecar attachment for code_review and validation under `hermes_gateway_real_dispatch`. The operator acceptance PR records the acceptance criteria and checklist an operator must review before any broader rollout. The next step is a controlled rollout gate.
