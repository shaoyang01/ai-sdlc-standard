...
**Recommended: Hermes Gateway Real Dispatch Observability**

Hermes Gateway real dispatch helper is now visible only as optional Gateway sidecar metadata under `hermes_gateway_real_dispatch`, with explicit `fallbackPolicy` classification for attach/omit decisions. The next step is sanitized sidecar observability without persisted audit logs or any primary Gateway/Runtime behavior change.
