...
def validate_expected(expected, id, label, errors)
  ...
  unless blockers.is_a?(Array) && blockers.all? { |b| b.is_a?(String) }
...
def validate_scenario(scenario, label, errors)
  ...
  errors << "#{label}: scenario #{id.inspect} input must be a Hash" unless input.is_a?(Hash)
...
def compute_tail_outcome(input, tmpdir)
  ...
  blockers << "tail status not completed" unless tail["status"] == "completed"
  ...
  if entry_coverage["applicable"] == true
    ...
    elsif !%w[current PASS].include?(entry_coverage["status"])
...
def run_self_tests(original)
  ...
  check_rejected = lambda do |label, tampered|
    errs = []
    ...
    begin
      ...
      errs << "self-test-#{label}: raised #{e.class}: #{e.message}"
    end
    failures << label if errs.empty?
  end
  ...
  check_rejected.call("tampered-expected-pass-fail", t1)
  ...
  check_rejected.call("missing-mandatory-tag", t2)
  ...
  check_rejected.call("unknown-scenario-field", t5)
  ...
  check_rejected.call("response-only-as-formal-pass", t6)
  ...
  check_rejected.call("write-failure-as-success", t7)
  ...
  check_rejected.call("readback-mismatch-as-success", t8)
  ...
  check_rejected.call("stale-evidence-as-pass", t9)
  ...
  check_rejected.call("wrong-route-as-pass", t10)
  ...
  check_rejected.call("v2-companion-as-pass", t11)
  ...
  check_rejected.call("critical-as-pass-with-risk", t12)

  # 12. YAML alias must be rejected at parse time.
  begin
    parse_fixture_text("base: &b 1\nchild: *b\n", "self-test-alias")
    failures << "yaml-alias-rejected"
  rescue Psych::Exception, RuntimeError
    nil # expected rejection
  end
  ...
  check_rejected.call("unsafe-canonical-source", t13)
...
def verify_fail_closed_markers(results, errors)
  checks = {
...
def main
  ...
  if errors.empty? && data
    selftest_failures = run_self_tests(data)
    errors.concat(selftest_failures)
  end

  if errors.empty? && data
    verify_fail_closed_markers(results, errors)
  end
