...
# has :header (normalized names), :rows (cells keyed by header), and :line.
...
def collect_strings(node, acc = [])
  ...
  when Array then node.each { |value| collect_strings(value, acc) }
  when Hash then node.each_value { |value| collect_strings(value, acc) }
  end
  acc
end
...
if repo_file?("system-capability-review.json")
  begin
    ...
    strings = collect_strings(review)
    [SHARED_FLOW_JSON, ARCHIVED_FLOW_REPORT].each do |path|
      unless strings.include?(path)
        errors << "shared-inventory: system-capability-review.json is missing accepted evidence path #{path}"
      end
    end
    OLD_EVIDENCE_PATHS.each do |old_path|
      if strings.include?(old_path)
        errors << "shared-inventory: system-capability-review.json still contains old root evidence path #{old_path}"
      end
    end
  rescue JSON::ParserError => e
