...
The output must decide what happens next: fix implementation, update specification, handle a requirement change, correct the test case, resolve environment/data issues, or hand reusable gaps to `test-feedback-sync`.
...
- `Return to specification-writer and rerun solution-reviewer`
- `Apply change-control`
- `Update test case or acceptance wording`
- `Resolve environment or data issue`
- `Run test-feedback-sync`
- `Run gate-runner`

Recommend `test-feedback-sync` only after the classification artifact is stable.
