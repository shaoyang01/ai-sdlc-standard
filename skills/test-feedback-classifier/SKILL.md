...
6. Do not update Checklist, Schema, or long-term knowledge; hand those needs to `test-feedback-sync`.
...
- Specification Missing -> return to `01-技术方案`, regenerate or revise specification, rerun `solution-reviewer`.
...
Use `docflow-writer` for HTML, Lark/Feishu, manifest writes, and output routing when requested. Keep this skill responsible for classification content only.
...
- Checklist or Schema update recommendation for `test-feedback-sync`
