...
12. Recommend fixes, Re-Gate, or `test-feedback-classifier` only when the review evidence supports it.
...
Use `docflow-writer` for HTML, Lark/Feishu, manifest writes, and output routing when requested. Keep this skill responsible for review report content only.
