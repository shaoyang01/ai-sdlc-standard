...
- `Return to requirement-normalizer` when the goal or scope changed.
- `Return to specification-writer` when the technical specification is missing or outdated.
- `Run solution-reviewer` when specification content changed and needs a new Specification Gate.
- `Continue to direct implementation` only when the Development Path Decision is `DIRECT_IMPLEMENTATION` and the Gate passes.
- `Run speckit-pipeline-confirmed-single` when the Development Path Decision is `SPECKIT_PIPELINE_REQUIRED`.
