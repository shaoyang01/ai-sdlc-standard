...
Run a generic DocFlow Gate check against a requirement manifest and related node artifacts. Treat this skill as a phase-entry auditor; do not replace specialized content reviewers such as `solution-reviewer`.
...
For HTML or Lark/Feishu output, use `docflow-writer` for routing and publishing. Keep this skill responsible for Gate evaluation content only.
