...
3. Do not review code quality as a replacement for `code-review-normalizer`.
...
12. Recommend `code-review-normalizer` as the next step only when no blocking issue remains.
...
Use `docflow-writer` for HTML, Lark/Feishu, manifest writes, and output routing when requested. Keep this skill responsible for implementation record content only.
