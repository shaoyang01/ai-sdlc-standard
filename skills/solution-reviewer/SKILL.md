...
description: |
  This skill should be used when the user asks to "审阅技术方案", "方案审阅", "检查方案是否能开发", "判断是否需要 Speckit", "审核 DeepSeek 方案", "输出方案审核", or asks Codex to review a technical specification and decide whether to implement directly or enter speckit-pipeline-confirmed-single.
version: 0.1.0
---

# Solution Reviewer

Review a technical specification as the global DocFlow Specification Gate. Decide whether the requirement can proceed to direct implementation, must enter `speckit-pipeline-confirmed-single`, or must return to specification revision.
...
If the user asks for HTML or Lark/Feishu output, use `docflow-writer` for routing and publishing. Keep this skill responsible for review content only.
