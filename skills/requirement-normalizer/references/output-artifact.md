...
- Updated At:
- Parsed By:
- Ready for specification-writer: yes/no
...
- Next Step: `Run specification-writer`, `Clarify requirement source`, or `Apply change-control and re-Gate`.

Do not silently edit `manifest.md` unless the user explicitly asks for file updates or output routing through `docflow-writer`.

## Readiness Values

Use `Ready for specification-writer: yes` only when no core-blocking uncertainty remains.

Use `Ready for specification-writer: no` when:
...
- Ready for `specification-writer`
