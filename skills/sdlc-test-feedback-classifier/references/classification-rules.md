...
| Specification Missing | Test exposes behavior that should be defined but is absent from `01-技术方案`. | Return to `01-技术方案`, create a new version, and rerun `sdlc-solution-reviewer`. |
| Review Missing | Code or plan issue should have been caught by review but was not. | Record review gap and recommend Checklist update through `sdlc-test-feedback-sync`. |
