...
description: |
  This skill should be used when the user asks to "同步到 specs/spec.md", "执行 speckit specify", "把已审阅方案转成 SpecKit spec", "从 01-技术方案 生成 specs", or asks to reuse DocFlow specification and solution-reviewer output as the Speckit specify source.
version: 0.1.0
---

# Speckit Specify Adapter
...
3. Do not replace `specification-writer`.
4. Do not replace `solution-reviewer`.
...
9. Require `solution-reviewer` result `PASS` or valid `PASS_WITH_RISK`.
