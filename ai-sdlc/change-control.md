...
| Superseded Artifact | 被新版本取代但必须保留的旧产物。 |
...
| 变更影响 | 受影响节点 | 必需动作 |
| --- | --- | --- |
| 原始需求或业务目标 | `00-需求资料` | 新增需求资料版本，重新确认需求。 |
| 技术方案、行为约束、异常、兼容、数据、接口、状态 | `01-技术方案` | 新增技术方案版本，重新方案审核。 |
| 方案审核结论或风险接受 | `02-方案审核` | 新增方案审核版本，重新 Gate。 |
| 实现范围、验证结果、未完成项 | `03-实现记录` | 新增实现记录版本，必要时重新代码审核。 |
| 代码审查发现阻塞问题 | `04-代码审核` | 修复后重新代码审核或更新审核结论。 |
| 测试反馈、验收口径、线上验证 | `05-测试验收` | 新增测试验收版本并分类处理。 |
...
```text
library/20260629-order-rule/01-技术方案/20260629-order-rule__技术方案__v1.html
library/20260629-order-rule/01-技术方案/20260629-order-rule__技术方案__v2.html
library/20260629-order-rule/02-方案审核/20260629-order-rule__方案审核__v2.html
```
...
- 新版本编号递增。
- 新版本必须说明变更来源和影响范围。
- 被替代的旧版本在 `manifest.md` 中标记为 superseded。
- 新 Gate 通过前，不能继续依赖旧 Gate 进入后续阶段。
...
6. Manifest 已记录旧版本 superseded 状态。
...
2. 新增 `01-技术方案` 新版本。
3. 新增 `02-方案审核` 新版本。
4. Manifest 标记旧技术方案和旧方案审核为 superseded。
...
| Date | Source | Type | Affected Node | Superseded | New Artifact | Required Re-Gate | Next Step |
| --- | --- | --- | --- | --- | --- | --- | --- |
```
