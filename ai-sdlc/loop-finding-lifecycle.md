...
| 3.0.0 | 2026-08-23 | Draft | C02-WP4 Round 2 复审修正（H2）：schema v4——finding 增加直接因果证据 `causeKind`（REGRESSION\|IMPROVEMENT，必填）与 `introducedByRevisionId`（REGRESSION 必填、须引用同 run revision）；Re-Gate 重启授权只消费该声明事实，不再以 source revision sequence 推断。风险接受载荷与 RISK_ACCEPTANCE proof 增加 `decisionScopeId`，绑定裁决轮的精确 Decision Scope；PASS_WITH_RISK 准入要求 ACCEPTED_RISK 关闭的 scope 与当前 verdict 的 decisionScopeId 完全相等。journal v7。 |
