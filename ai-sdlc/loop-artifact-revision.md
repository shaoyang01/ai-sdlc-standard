...
- manifest status 与 runtime validity 按**冻结映射表**（主合同 §6.2.4，G4-R5-M2 对齐）一致：revision `ACTIVE` ↔ `current`；`STALE`/`SUPERSEDED` ↔ `stale`（手动面可在非 current 行上以 `actionable` 标注回流待返工节点，语义同非 current）。冻结前的 `draft`/`active`/`replaced` 词表废止，携带即 `INVALID_INPUT`；
