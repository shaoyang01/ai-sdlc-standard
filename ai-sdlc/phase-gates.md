...
### Development Path Entry Check

进入实现前的路径准入检查，遵循 `ai-sdlc/development-path-governance.md`：

- Specification Gate 当前有效。
- Development Path Decision 存在。
- Complexity、Decision Scope 及 rationale 当前有效。
- `DIRECT_IMPLEMENTATION` 或 `SPECKIT_PIPELINE_REQUIRED` 与当前实现范围一致。
- Tail required 和 scope 已确定。
- `BLOCKED_NEEDS_REVISION` 路径不得进入实现。
...
Tail Completion Gate 遵循 `ai-sdlc/development-path-governance.md`：

- owner 为 `sdlc-gate-runner`，只检查证据，不执行专业工作。
- 产生实际代码、配置或行为实现时，要求当前有效的 `03-实现记录`、`04-代码审核`、`05-测试验收`。
- 要求当前有效的 business_domain_sync decision。
- 要求当前有效的 Reconcile decision。
- 要求适用的 entry coverage 与 Re-Gate。
- stale 或缺失 evidence 必须阻塞。
- Manifest 为 Tail 状态权威。
- Stage Summary 不是 Gate；Pipeline COMPLETED 不是 Tail Completion Gate。
